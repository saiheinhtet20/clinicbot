...
app.get('/admin/doctorappointments', async function(req,res){
  const doctorappointmentsRef = db.collection('doctorappointments');
  const snapshot = await doctorappointmentsRef.get();
  if(snapshot.empty){
    res.send('no data');
  }
  ...
  snapshot.forEach(doc => {
    ...
    doctorappointment = doc.data();
    doctorappointment.doc_id = doc.id;

    data.push(doctorappointment);
    
  });

  console.log('DATA:', data);

  res.render('doctorappointments.ejs', {data:data});
});

app.get('/admin/updatedoctorappointment/:doc_id', async function(req,res){
  ...
  if(received_message.startsWith("visit:")){
    ...
    switch(received_message) {     
        ...
        case "off":w
          ...
          break;   
        case "confirm-doctorappointment":
...
const appointment =(sender_psid) => {
  let response1 = {"text": "Welcome to ThawdarWinClinic"};
  let response2 = {
    "text": "Please Select Doctor or Medicine",
    "quick_replies":[
            {
              "content_type":"text",
              "title":"Doctor",
    ...
    "quick_replies":[
            {
              "content_type":"text",
              "title":"Confirm",
              "payload":"confirm-doctorappointment",              
            },{
              "content_type":"text",
              "title":"Cancel",
              "payload":"off",             
...
const saveDoctorBooking = async (arg, sender_psid) =>{
  ...
  data.status = "pending";
  db.collection('doctorappointments').add(data).then((success)=>{
